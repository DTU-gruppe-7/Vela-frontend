...
export default function InviteGroupModal({ isOpen, onClose, groupName }: InviteGroupModalProps) {
    ...
    const handleSendInvite = async (e: React.FormEvent) => {
        ...
        // Simulerer et netværkskald til jeres C# backend
        await new Promise(resolve => setTimeout(resolve, 1200));
        
        console.log(`Invitation sendt til ${email} for gruppen ${groupName}`);
        setIsSending(false);
        setIsSuccess(true);

        // Nulstil efter 2 sekunder og luk eller lad brugeren sende en til
        setTimeout(() => {
            setIsSuccess(false);
            setEmail('');
        }, 2500);
